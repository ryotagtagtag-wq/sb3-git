...
var importCommand = new Command("import").description("Convert .sb3 file to expanded intermediate format").argument("<input>", "Input .sb3 file path").option("-o, --output <dir>", "Output directory (default: input name + .expanded)").option("--no-assets", "Skip asset extraction").action(async (input, options) => {
  ...
  const withIds = assignSemanticIds2(normalized);
...
var exportCommand = new Command2("export").description("Convert expanded intermediate format to .sb3 file").argument("<input>", "Input directory (expanded format)").argument("<output>", "Output .sb3 file path").action(async (input, output) => {
  ...
  const buffer = await exportSB3(project, assets);
  await writeFile3(output, buffer);
...
// src/cli/commands/diff.ts
var diffCommand = new Command3("diff").description("Generate HTML diff report between two SB3 files or expanded projects").argument("<base>", "Base SB3 file or expanded project directory").argument("<modified>", "Modified SB3 file or expanded project directory").option("-o, --output <file>", "Output HTML file (default: diff-report.html)").option("--json", "Output JSON diff instead of HTML").option("--base-label <label>", "Label for base version", "Base").option("--new-label <label>", "Label for modified version", "Modified").action(async (base, modified, options) => {
  ...
  if (isBaseSb3 && isModifiedSb3) {
    ...
    const baseProj = assignSemanticIds2(baseNormalized);
    const modifiedProj = assignSemanticIds2(modifiedNormalized);
  ...
  await writeFile4(outputFile, html);
  ...
  for (const target of diff.targets) {
    ...
    totalChanges += target.comments.filter((c) => c.type !== "unchanged").length;
    totalChanges += target.costumes.filter((c) => c.type !== "unchanged").length;
    totalChanges += target.sounds.filter((s) => s.type !== "unchanged").length;
    totalChanges += target.properties.length;
  }
  console.log(`
Summary: ${totalChanges} changes detected`);
  console.log(`  Targets: ${diff.targets.length}`);
  console.log(`  Monitors: ${diff.monitors.filter((m) => m.type !== "unchanged").length} changed`);
  console.log(`  Extensions: ${diff.extensions.filter((e) => e.type !== "unchanged").length} changed`);
});
...
function mergeBlock(base, ours, theirs, conflicts, autoResolve) {
  const baseHash = computeBlockContentHash2(base);
  const ourHash = computeBlockContentHash2(ours);
  const theirHash = computeBlockContentHash2(theirs);
...
// src/cli/commands/merge.ts
var mergeCommand = new Command4("merge").description("Three-way merge of SB3 projects").argument("<base>", "Base .sb3 file").argument("<ours>", "Our .sb3 file").argument("<theirs>", "Their .sb3 file").argument("<output>", "Output .sb3 file").option("--auto-resolve", "Automatically resolve conflicts preferring ours").action(async (base, ours, theirs, output, options) => {
  console.log(`Three-way merge:`);
  console.log(`  Base: ${base}`);
  console.log(`  Ours: ${ours}`);
  console.log(`  Theirs: ${theirs}`);
  console.log(`  Output: ${output}`);
  const baseBuffer = readFileSync2(base);
  const oursBuffer = readFileSync2(ours);
  const theirsBuffer = readFileSync2(theirs);
  const baseSb3Project = await importSB3(baseBuffer);
  ...
  const baseProj = assignSemanticIds2(baseNormalized);
  const oursProj = assignSemanticIds2(oursNormalized);
  const theirsProj = assignSemanticIds2(theirsNormalized);
...
Auto-resolving conflicts preferring ours...`);
  ...
  const mergedAssets = /* @__PURE__ */ new Map();
...
// src/cli/commands/validate.ts
var validateCommand = new Command5("validate").description("Validate expanded project integrity").argument("<input>", "Input directory (expanded format)").option("--strict", "Exit with error code on warnings").action(async (input, options) => {
...
var watchCommand = new Command6("watch").description("Watch for changes and show live diff (stub)").argument("<input>", "Input directory to watch").option("-p, --port <number>", "Port for live server", "3000").action((input, options) => {
...
program.name("sb3-git").description("Git/GitHub collaborative development for Scratch .sb3 projects").version("0.1.0");
